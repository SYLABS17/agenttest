"""FastAPI application for Azure LMS."""
...
from src.pipeline import AzureLMSPipeline
from src.pipeline.orchestrator import QueryRequest

logger = structlog.get_logger(__name__)

_pipeline: Optional[AzureLMSPipeline] = None
...
async def lifespan(app: FastAPI):
    ...
    global _pipeline
    logger.info("starting_azure_lms")
    _pipeline = AzureLMSPipeline()
    yield
    logger.info("shutting_down_azure_lms")


def create_app() -> FastAPI:
    """Create FastAPI application."""
    application = FastAPI(
        title="National LMS API - Azure",
        description="Multi-modal RAG API powered by Azure AI services",
        version="1.0.0",
        lifespan=lifespan,
    )
...
async def answer_query(request: QueryRequestModel):
    """Answer a student's educational query using Azure services."""
...
async def health_check():
    ...
    if _pipeline is None:
        return {"status": "unhealthy", "cloud": "azure"}
...
async def readiness_check():
    """Readiness probe."""
    return {"ready": _pipeline is not None, "cloud": "azure"}


@app.get("/health/live")
async def liveness_check():
    """Liveness probe."""
    return {"alive": True, "cloud": "azure"}
