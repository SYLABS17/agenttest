"""Video segment retrieval for AWS."""
...
class VideoRetriever:
    """Video segment retrieval service using S3."""
        ...
        for chunk in chunks:
            ...
            if video_id and video_id not in seen_videos:
                segment = VideoSegment(
                    ...
                    url=f"https://{self.settings.s3_bucket_name}.s3.{self.settings.aws_region}.amazonaws.com/videos/{video_id}",
                    thumbnail_url=f"https://{self.settings.s3_bucket_name}.s3.{self.settings.aws_region}.amazonaws.com/{self.settings.s3_frames_prefix}{video_id}/thumb.jpg",
                )
