"""FastAPI application for GCP LMS."""
...
from src.pipeline import GCPLMSPipeline
from src.pipeline.orchestrator import QueryRequest

logger = structlog.get_logger(__name__)

_pipeline: Optional[GCPLMSPipeline] = None
...
async def lifespan(app: FastAPI):
    ...
    global _pipeline
    logger.info("starting_gcp_lms")
    _pipeline = GCPLMSPipeline()
    yield
    logger.info("shutting_down_gcp_lms")


def create_app() -> FastAPI:
    """Create FastAPI application."""
    application = FastAPI(
        title="National LMS API - GCP",
        description="Multi-modal RAG API powered by Google Cloud AI services",
        version="1.0.0",
        lifespan=lifespan,
    )
...
async def answer_query(request: QueryRequestModel):
    """Answer a student's educational query using GCP services."""
...
async def health_check():
    ...
    if _pipeline is None:
        return {"status": "unhealthy", "cloud": "gcp"}
...
async def readiness_check():
    """Readiness probe."""
    return {"ready": _pipeline is not None, "cloud": "gcp"}


@app.get("/health/live")
async def liveness_check():
    """Liveness probe."""
    return {"alive": True, "cloud": "gcp"}
