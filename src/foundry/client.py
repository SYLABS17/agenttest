...
class AzureAIFoundryClient:
    ...
    async def initialize(self):
        ...
        search_conn = self.project.connections.get_default(
        ...
        self._agent = self.project.agents.create_agent(
            ...
            name="lms-assistant",
            instructions="You are a helpful learning assistant. Answer questions accurately based on the provided content. Always cite sources.",
            tools=[search_tool],
        )

    async def query(self, question: str, language: str = "en") -> QueryResult:
        ...
        self.project.agents.create_and_process_run(
            thread_id=thread.id, assistant_id=self._agent.id
        )
        ...
        if not assistant_msg:
            return QueryResult(answer="Unable to process.", sources=[], confidence=0.0)

        answer = assistant_msg.content[0].text.value
        annotations = getattr(assistant_msg.content[0].text, "annotations", [])
        sources = [{"quote": a.text} for a in annotations if hasattr(a, "file_citation")]

        return QueryResult(answer=answer, sources=sources, confidence=0.9 if sources else 0.7)
